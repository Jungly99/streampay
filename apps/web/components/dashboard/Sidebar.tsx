    ...
    <aside style={{
      ...
      <div style={{ margin: '0 10px 10px', padding: '12px 14px', borderRadius: 12, background: channelCardBg, border: `1px solid ${channelCardBorder}`, flexShrink: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          <div style={{ background: statBg1, borderRadius: 8, padding: '7px 10px', border: '1px solid rgba(124,58,237,0.12)' }}>
            <p style={{ fontSize: 13, fontWeight: 800, color: '#a78bfa', margin: 0 }}>₹{todayEarnings}</p>
            <p style={{ fontSize: 10, color: textMuted, margin: 0, marginTop: 1 }}>Today</p>
          </div>
          <div style={{ background: statBg2, borderRadius: 8, padding: '7px 10px', border: `1px solid ${channelCardBorder}` }}>
            <p style={{ fontSize: 13, fontWeight: 800, color: textPrimary, margin: 0 }}>{followers}</p>
            <p style={{ fontSize: 10, color: textMuted, margin: 0, marginTop: 1 }}>Fans</p>
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px', display: 'flex', flexDirection: 'column' }}>
        {navGroups.map((group, gi) => (
          <div key={gi} style={{ marginBottom: 4 }}>
            {gi > 0 && <div style={{ height: 1, background: dividerBg, margin: '6px 6px 8px' }} />}
            {group.items.map(item => {
              const on = isActive(item.href)
              const inner = (
                <>
                  <div style={{ width: 28, height: 28, borderRadius: 7, flexShrink: 0, background: on ? item.color : `${item.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: on ? 'white' : item.color, transition: 'all 0.15s', fontWeight: 700 }}>
                    {item.symbol}
                  </div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <span style={{ display: 'block', fontSize: 12.5, fontWeight: on ? 700 : 500, color: on ? textPrimary : (isDark ? '#8b9ab0' : '#4c4a82'), lineHeight: 1.3 }}>{item.label}</span>
                    <span style={{ display: 'block', fontSize: 10, color: on ? `${item.color}cc` : textFaint, marginTop: 1 }}>{item.sub}</span>
                  </div>
              ...
              const clickProps = onClose ? { onClick: onClose } : {}
              return item.external ? (
                <a key={item.href} href={item.href} target="_blank" rel="noopener noreferrer" style={style}>{inner}</a>
              ) : (
                <Link key={item.href} href={item.href} style={style} {...clickProps}>{inner}</Link>
              )
            })}
          </div>
        ))}
        ...
        {/* Quick actions */}
        <div style={{ height: 1, background: dividerBg, margin: '6px 6px 10px' }} />
        <p style={{ fontSize: 9, fontWeight: 700, color: textFaint, letterSpacing: '0.12em', paddingLeft: 8, marginBottom: 8 }}>QUICK ACTIONS</p>
          ...
          {overlayUrl && (
            <a href={overlayUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none', padding: '8px 10px', borderRadius: 10, background: 'rgba(14,165,233,0.06)', border: '1px solid rgba(14,165,233,0.12)', transition: 'all 0.15s' }}>
              ...
              <div>
                ...
                <span style={{ display: 'block', fontSize: 10, color: textFaint }}>For OBS Studio</span>
              </div>
            </a>
          )}
          {messageLink && (
        ...
        {/* Platform info card */}
        <div style={{ margin: '0 2px 8px', padding: '14px 14px', borderRadius: 12, background: platformCardBg, border: `1px solid ${platformCardBorder}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={s.label} style={{ background: statItemBg, borderRadius: 7, padding: '6px 8px' }}>
                <p style={{ fontSize: 9, color: textFaint, margin: 0, letterSpacing: '0.05em', textTransform: 'uppercase', fontWeight: 600 }}>{s.label}</p>
                <p style={{ fontSize: 12, fontWeight: 700, color: '#a78bfa', margin: '2px 0 0' }}>{s.value}</p>
              </div>
            ))}
          </div>
          <a href="mailto:support@eztips.live" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontSize: 11, color: textMuted, textDecoration: 'none', padding: '6px', borderRadius: 8, background: emailBg, border: `1px solid ${emailBorder}` }}>
      ...
      <div style={{ padding: '8px 10px 10px', borderTop: `1px solid ${dividerBg}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 6px', marginBottom: 4 }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <p style={{ fontSize: 9, color: textFaint, margin: 0, letterSpacing: '0.05em' }}>STREAMER ACCOUNT</p>
          </div>
        </div>
        <form action="/api/auth/logout" method="POST">
          <button type="submit" style={{ width: '100%', padding: '8px 12px', borderRadius: 9, cursor: 'pointer', background: 'transparent', border: '1px solid rgba(248,113,113,0.1)', color: textMuted, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s' }}
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(248,113,113,0.1)'; e.currentTarget.style.color=textMuted; e.currentTarget.style.background='transparent' }}>
