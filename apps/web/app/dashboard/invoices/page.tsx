...
export default function InvoicesPage() {
  ...
  return (
    <div style={{ padding: 28, minHeight: '100%', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-1)', letterSpacing: '-0.5px' }}>Invoices</h1>
        <p style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 3 }}>Monthly GST-compliant invoices for your settlements</p>
      </div>
      ...
      <div style={{ ...C, padding: '14px 18px', background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.15)' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-3)' }}>Invoices are auto-generated on the 1st of each month after any settlement in the previous month. All invoices are GST-compliant and usable for tax filing.</p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
          {[
            ...
            <div key={s.label} style={{ ...C, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-1)' }}>{s.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ ...C, overflow: 'hidden', flex: 1 }}>
        {filtered.length === 0 ? (
          <div style={{ padding: '60px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-1)', marginBottom: 8 }}>No invoices yet</p>
            <p style={{ fontSize: 13, color: 'var(--text-3)' }}>Invoices appear after your first settlement. Settle your earnings to generate your first invoice.</p>
          </div>
        ) : (
            ...
            <tbody>
              {filtered.map((inv: any, i: number) => (
                <tr key={inv.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                  ...
                  <td style={{ padding: '14px 18px', fontSize: 12, color: 'var(--text-3)' }}>{formatDate(inv.periodStart)} – {formatDate(inv.periodEnd)}</td>
                  ...
                  <td style={{ padding: '14px 18px', fontSize: 12, color: 'var(--text-3)' }}>{formatDate(inv.generatedAt)}</td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ ...C, padding: '20px 22px' }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', marginBottom: 14 }}>For Tax Filing</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={item.text} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-3)', margin: 0, lineHeight: 1.5 }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
