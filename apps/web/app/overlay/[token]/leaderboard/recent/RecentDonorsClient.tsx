...
export default function RecentDonorsClient({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch(`${BACKEND}/api/donations/overlay-leaderboard/${token}`)
      .then(r => r.json())
      .then(d => setDonors(
        (d.recentDonors ?? []).map((r: any, i: number) => ({ name: r.name, amount: r.amount, id: `init-${i}` }))
      ))
      .catch(() => {})
  ...
  return (
    <div style={{ background: 'transparent', padding: 8, minWidth: 240 }}>
      ...
      <div style={{
        ...
        borderRadius: 16, padding: '14px 16px', border: '1px solid rgba(16,185,129,0.3)',
        boxShadow: '0 4px 32px rgba(0,0,0,0.5)',
      }}>
        <p style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 800, color: '#10b981', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
          👥 Recent Donors
        </p>
            ...
            {donors.map((d, i) => (
              <motion.div
                ...
                style={{
                  ...
                  border: `1px solid ${i === 0 ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.06)'}`,
                  opacity: Math.max(1 - i * 0.15, 0.4),
                }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: i === 0 ? '#34d399' : '#94a3b8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>{d.name}</span>
                <span style={{ fontSize: 13, fontWeight: 800, color: i === 0 ? '#10b981' : '#64748b', flexShrink: 0, marginLeft: 8 }}>₹{d.amount.toLocaleString('en-IN')}</span>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
